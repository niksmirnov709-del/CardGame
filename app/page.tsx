...
function attackPower(card: Card, suddenDeath: boolean) {
  return card.kind === 'shield' && suddenDeath ? 2 : card.value;
}
...
function canDefend(card: Card, attack: Card, trump: Suit, suddenDeath: boolean) {
  ...
  if (attack.kind === 'shield') return suddenDeath && (card.value > 2 || card.suit === trump);
...
function canChain(card: Card, previous: Card, suddenDeath: boolean) {
  const playable = card.kind === 'strike' || suddenDeath;
  return playable && (card.suit === previous.suit || attackPower(card, suddenDeath) === attackPower(previous, suddenDeath));
}
...
export default function Home() {
  ...
  const playAttack = () => {
    ...
    const playable = card && (card.kind === 'strike' || game.suddenDeath);
  ...
  const resolveDefense = (defense: Card | null) => {
    ...
    if (blocked) {
      ...
      const damage = Math.max(2, Math.ceil((attackPower(currentAttack, game.suddenDeath) + game.attackBonus) / 3)) + chainBonus;
  ...
  const selectedIsAttack = !!selectedCard && (selectedCard.kind === 'strike' || game.suddenDeath);
      ...
      <section className="player-zone">
        ...
        <div className="player-hand" aria-label={`Mano de ${self.name}`}>
          {self.hand.map((card, index) => {
            ...
            const playableAttack = card.kind === 'strike' || game.suddenDeath;
        ...
        <div className="action-dock">
          {(game.phase === 'attack' || game.phase === 'chain') && <>
            ...
            <button className="play-button" disabled={!canAttack} onClick={playAttack}>{secretChoice ? 'Jugar boca abajo' : game.phase === 'chain' ? `Redoblar ×${game.attacks.length + 1}` : game.players[game.attacker].pulse === 3 ? 'Atacar con +2' : game.suddenDeath && selectedCard?.kind === 'shield' ? 'Embestida 2' : 'Jugar ataque'}</button>
          </>}
          {game.phase === 'defend' && <>
